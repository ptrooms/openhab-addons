...
import static org.openhab.binding.avmfritz2.internal.ahamodel.DeviceModel.ETSUnitInfo.*;
...
import org.openhab.binding.avmfritz2.internal.BindingConstants;
import org.openhab.binding.avmfritz2.internal.ahamodel.AVMFritzBaseModel;
import org.openhab.binding.avmfritz2.internal.ahamodel.AlertModel;
import org.openhab.binding.avmfritz2.internal.ahamodel.DeviceModel;
import org.openhab.binding.avmfritz2.internal.ahamodel.GroupModel;
...
public abstract class AVMFritzBaseBridgeHandler extends BaseBridgeHandler {
     ...
     */
    private @Nullable ScheduledFuture<?> pollingJob;		// gives warning
    ...
    public AVMFritzBaseBridgeHandler(Bridge bridge, HttpClient httpClient) {
        super(bridge);		// The super keyword refers to superclass (parent) 
        this.httpClient = httpClient;
    }

    /**
     * Initializes the bridge. // ptro called when bundle is started (I presume)
     */
    @Override
    public void initialize() {
		pollCount = 0;		// ptro 04mar24 reset at start
        logger.warn("About to initialize FRITZ!Box {} , pollcount={}", BRIDGE_FRITZBOX, pollCount);	// ptro 04mar24 added pollcount in message
        AVMFritzConfiguration config = getConfigAs(AVMFritzConfiguration.class);

        logger.debug("Discovered FRITZ!Box initialized: {}", config);
		// org.openhab.binding.avmfritz2.internal.config.AVMFritzConfiguration@9cc81e[
		//	IP=192.168.1.2,
		//		port=<null>,proto=http,user=xxxxx,
		//		password=xxxxx,
		//		pollingInterval=10,
		//		asyncTimeout=10000,
		//		syncTimeout=2000
        ...
        this.connection = new FritzAhaWebInterface(config, this, httpClient); 		// --> ../hardware/FritzAhaWebInterface.java
			// --> public FritzAhaWebInterface(AVMFritzConfiguration config, AVMFritzBaseBridgeHandler handler,HttpClient httpClient)
			// "connection" does not return in case of any timeout or error on settingup the httpClient
				// this keyword refers to the current object in a method or constructor.
				// httpClient --> from org.eclipse.jetty.client.HttpClient	
				// FritzAhaWebInterface --> org.openhab.binding.avmfritz2.internal.hardware.FritzAhaWebInterface;

        if (config.getPassword() != null) {
	        logger.debug("ptro initialize config.getPassword = null , do onUpdate");		// ptro 04mar24
            onUpdate();
        } else {
	        logger.debug("ptro initialize ThingStatus.OFFLINE , do updateStatus");
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, "no password set"); 		// ptro 04mar24
        }
    }
    ...
    private void poll() {
        ...
        if (webInterface != null) {
            ...
            logger.debug("polling {} FRITZ!Box {}", pollCount, getThing().getUID()); // display pollCpunter in message

// test singlecall use same instance
            FritzAhaUpdateXmlCallback callback = localFritzAhaUpdateXmlCallback;		// define
			if (pollCount == 0) {														// and get initial
	            localFritzAhaUpdateXmlCallback = new FritzAhaUpdateXmlCallback(webInterface, this);
			}
            callback = localFritzAhaUpdateXmlCallback;									// overwrite
/*
// test multicall each instance is new
			FritzAhaUpdateXmlCallback callback = new FritzAhaUpdateXmlCallback(webInterface, this);
			localFritzAhaUpdateXmlCallback = callback;
*/
            webInterface.asyncGet(callback);
			pollCount++; 	// ptro 04mar24 increase call count
        } else {
            logger.debug("webInterface = null at polling {} FRITZ!Box {}", pollCount, getThing().getUID()); // display pollCpunter in message
		}
    }
    ...
    public void handleRefreshCommand() {
        logger.debug("handleRefreshCommand");
