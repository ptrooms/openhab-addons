 ...
 */
package org.openhab.binding.avmfritz2.internal.discovery;
...
/**
 * The {@link AVMFritzUpnpDiscoveryParticipant2} is responsible for discovering new and removed FRITZ!Box devices. It
 ...
 */
@Component(immediate = true, configurationPid = "discovery.avmfritz2")
@NonNullByDefault
public class AVMFritzUpnpDiscoveryParticipant2 implements UpnpDiscoveryParticipant {

    private final Logger logger = LoggerFactory.getLogger(AVMFritzUpnpDiscoveryParticipant2.class);
                ...
                if (modelDetails != null) {
                    ...
                    if (modelName != null) {
                        ...
                        if (modelName.startsWith(BOX_MODEL_NAME)) {
                            logger.debug("discovered2a on {}", device.getIdentity().getDiscoveredOnLocalAddress());	// ptro 04mar23 diff
                            return new ThingUID(BRIDGE_THING_TYPE, id);
                        } else if (modelName.startsWith(POWERLINE_MODEL_NAME)) {
                            logger.debug("discovered2b on {}", device.getIdentity().getDiscoveredOnLocalAddress()); // ptro 04mar23 diff
