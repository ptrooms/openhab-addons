...
public abstract class AVMFritzBaseThingHandler extends BaseThingHandler {
    ...
    public AVMFritzBaseModel getState() {
		if (state == null) {			// 03mar24 ptro: ifadded to check see what happens if null
            logger.debug("state = null for void AVMFritzBaseModel");
		}
        return state;
    }
    ...
    public void setState(AVMFritzBaseModel state) {
		if (state == null) {			// 03mar24 ptro: ifadded to check see what happens if null
            logger.debug("state = null at setState");
		}
        this.state = state;
    }
}
